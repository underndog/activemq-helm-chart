...
| Parameter | Description | Default |
|-----------|-------------|---------|
| `initContainers` | Init containers configuration | See [values.yaml](values.yaml) |
...
- `bootstrap.xml` - Web server configuration
- `management.xml` - JMX management configuration
- `login.config` - JAAS login configuration
- `artemis-users.properties` - User definitions
- `artemis-roles.properties` - Role definitions
- `jmx.access` - JMX access control
- `jmx.password` - JMX passwords
- `broker.properties` - Additional broker properties

You can customize these files by modifying the ConfigMap template.
...
│   ├── ebs-gp3-sc.yml         # Storage class definition
